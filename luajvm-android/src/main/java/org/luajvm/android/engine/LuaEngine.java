...
public class LuaEngine implements LuaContext {
    ...
    public Object runFunc(String name, Object... args) {
        // 引擎未启动（域外脚本等待信任确认）时生命周期转发为空操作
        if (mGlobals == null) return null;
        try {
    ...
    public void sendError(String title, Exception e) {
        ...
        // 宿主实例级列表：Activity 的错误页只显示本实例的错误（见 LuaHost.onScriptError）
        if (mHost instanceof LuaHost host) host.onScriptError(title, e);
