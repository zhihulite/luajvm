...
├── luajvm-core-android/  # core 的 Android 兼容版：core 源码直编 + jar 期字节码重写
│   └── src/main/java/org/luajvm/android/compat/  # JavaApiCompat（超版本 API 等价实现）
...
`luajvm-core` 和 `luajvm-android` 必须同版本发布：`luajvm-android` 以 `api` 依赖 `luajvm-core-android`（core 的 Android 兼容版），版本对不上会让用到的人 classpath 上出现两份不同的 core。三者的 `group`/`version` 统一在根 `build.gradle` 读取 `gradle.properties`，不在各模块单独声明。

发布门禁按模块分层：`luajvm-core` 发布前强制跑核心测试套件（officialTests/luajvmTests/javaApiTests）；
`luajvm-core-android` 发布前强制跑 `sniffArtifact`（Animal Sniffer 对重写后的 jar 做 Android API 24
签名核对，超版本 API 引用直接失败——机制见 `docs/android-compat.md`）；`luajvm-android` 发布前强制跑
`lintRelease`。门禁不过制品不出仓。

根 `build.gradle` 的 `publishableModules` 白名单只有 core、core-android 和 android 可以发布，不在名单里的模块一旦应用 `maven-publish` 构建会失败。模块的 `src/test` 里有大量门禁代码与 Lua 套件，但 test 源集不进制品（`withSourcesJar` 只打 main）。
...
| `docs/GATES.md` | 当前门禁表 |
| `docs/android-compat.md` | Android 兼容层：三层制品、字节码重写、API 24 签名核对门禁、协程 SPI |
