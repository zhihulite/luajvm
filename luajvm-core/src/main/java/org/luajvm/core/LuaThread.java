...
public class LuaThread extends LuaValue {
    ...
    // java-only: 协程线程创建经 CoroutineThreads SPI——默认实现是虚拟线程（Java 21），
    //   Android 宿主注入平台线程实现；core 不含 vm.name 等平台探测
    private static final CoroutineThreads COROUTINE_THREADS = loadCoroutineThreads();
    // java-only: 平台线程的原生栈大小。0 表示使用平台默认值；
    ...
    /** 测试与日志查询当前协程线程实现形态。 */
    public static boolean coroutineThreadsIsVirtual() {
        return COROUTINE_THREADS.isVirtual();
    }

    private static CoroutineThreads loadCoroutineThreads() {
        for (var it = ServiceLoader.load(CoroutineThreads.class).iterator(); it.hasNext(); ) {
            return it.next();
        }
        // 无注入时默认：虚拟线程。Thread.ofVirtual 只出现在这里（本默认实现）
        return new CoroutineThreads() {
            @Override
    ...
    public Varargs lua_resume(Varargs args) {
        ...
        try {
            ...
            if (useCont()) {
                ...
                // daemon 由实现负责（虚拟线程恒为 daemon；平台线程在实现里 setDaemon）
                COROUTINE_THREADS.start(this::runCoroutine, "LuaCoroutine", PLATFORM_THREAD_STACK);
            } else {
